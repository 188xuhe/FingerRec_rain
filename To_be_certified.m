...
% close all
M=0;var=0;
I2=imread('G:\ZKFinger SDK 5.0.0.20_20170424\ActiveX\samples\VC\Debug\fingerprint.bmp');
[m,n] = size(I2);
I = double(I2);
%图像归一化
for x=1:m 
...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 中心点的提取
 if(mod((mxmin+mxmax),2)==0)
      l=(mxmin+mxmax)/2;
 else 
      l=((mxmin+mxmax)+1)/2;
 end
 if(mod((mymin+mymax),2)==0)
      d=(mymin+mymax)/2;
 else 
      d=((mymin+mymax)+1)/2;
 end
...
%  对第一个扫描点做独立的判断：           
            ...
            while(~(tt==1|(type==1&(count1-number)>=(-1)&md==0)|mt==1)&(X1(x,y)==1))%情况判别：  1.存在分支情况，用tt判别    2.图像采集顺利，单条下来     3.独立点用mt判别        &md==0
                %对扫描点x，y进行范围设定
                 if((x>=(d+100)|x<=(d-100)|y>=(l+75)|y<=(l-75)))  
...
sanjiaoxingshu1=(zongshu*(zongshu-1)*(zongshu-2)/6)  %记录三角形个数  C （ZONGSHU）(3)  排列组合
% % % % % % % % % % % % % % % % % % %
NN2=zeros(sanjiaoxingshu1,3);
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 三角配对
jl=0;%记录三角形个数的
for i=1:(zongshu-2)
    for j=(i+1):(zongshu-1)
        for k=(j+1):zongshu
          jl=jl+1;
          NN2(jl,1)=sqrt((MN(i,1)-MN(j,1))^2+(MN(i,2)-MN(j,2))^2) ;
          NN2(jl,2)=sqrt((MN(i,1)-MN(k,1))^2+(MN(i,2)-MN(k,2))^2) ;
          NN2(jl,3)=sqrt((MN(j,1)-MN(k,1))^2+(MN(j,2)-MN(k,2))^2) ;
        end
    end
end
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
yuzhi1=(yuzhi*(yuzhi-1)*(yuzhi-2)/6);%用于计入匹配的三角形的最小个数
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
anwser=0;anwser1=0;
yuzhi3=0.65;   %阈值3  只要用来刊期匹配度的   用于调整 
% % % % % % % % % 根据特征点的点数来设置阈值
if(zongshu>=22)
    yuzhi2=0.04;
    else if(zongshu>=20)
...
canshu1=1-yuzhi2;% 设置阈值
canshu2=1+yuzhi2;
% % % % % % % % % % % % % % % % % 
for i=1:sanjiaoxingshu1
    for j=1:sanjiaoxingshu
        if(((NN2(i,1)>=(canshu1*NN(j,1)))&(NN2(i,1)<=(canshu2*NN(j,1))))&((NN2(i,2)>=(canshu1*NN(j,2)))&(NN2(i,2)<=(canshu2*NN(j,2))))&((NN2(i,3)>=(canshu1*NN(j,3)))&(NN2(i,3)<=(canshu2*NN(j,3)))))
              anwser1=anwser1+1;
              break;
        end
    end
end
anwser1=anwser1/sanjiaoxingshu1;
if(anwser1>yuzhi3)
    anwser=1;
end
anwser2=zongshu;
